...
# ─── ВАЛИДАЦИЯ КАРТИНОК ──────────────────────────────────────────────
# data URL до 200 KB; разрешённые MIME — image/jpeg|png|webp.
...
def create_message(user_id, text, kind='message', images=None, mentions=None):
    ...
    with db() as conn:
        conn.execute(
            ...
            '(id, user_id, kind, text, is_deleted, created_at, updated_at) '
            'VALUES (?, ?, ?, ?, 0, ?, ?)',
            (msg_id, user_id, kind, text, now, now),
        )
...
def toggle_reaction(message_id, user_id, emoji):
    ...
    with db() as conn:
        ...
        if existing:
            ...
            action = 'added'
        logger.info('[COMMUNITY][REACT] msg=%s uid=%s emoji=%r %s',
