/* views/docs.js — модуль раздела «Документация» (шаг 0.5d).
   Перенесён из inline-блока static/index.html (~1688..1726).

   Сейчас содержит ТОЛЬКО функции таблицы кодов логов внутри страницы
   документации (loadDocsLogCodes + renderLogCodesHtml). Полноразмерный
   loadLogCodes() для отдельного view #view-logcodes пока остаётся inline,
   так как использует свой собственный набор иконок и категорий — он
   будет вынесен в отдельный шаг.
...
(function(){
  ...
  var docsLogCodesLoaded = false;

  function loadDocsLogCodes(){
    if (docsLogCodesLoaded) return;
    docsLogCodesLoaded = true;
    var cont = _q('docsLogCodesContainer');
    if (!cont) return;
    _api()('/api/log-codes').then(function(d){
      var codes = d.codes || [];
      cont.innerHTML = renderLogCodesHtml(codes) || '<div style="color:#555;font-size:13px">Нет данных</div>';
    }).catch(function(){
      cont.innerHTML = '<div style="color:#666;font-size:13px">Ошибка загрузки</div>';
    });
  }

  function renderLogCodesHtml(codes){
    var groups = {};
    ...
    var catIcons = {
      ...
      'Спонсоры':  '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg>',
      'Модели':    '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/></svg>',
      'Управление':'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/></svg>',
    };
  ...
  /* ── Экспорт в window.* ── */
  window.loadDocsLogCodes  = loadDocsLogCodes;
  window.renderLogCodesHtml = renderLogCodesHtml;
})();
